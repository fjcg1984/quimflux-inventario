...
(function(){
  ...
  function nextCode(category){
    ...
    (window.state?.items||[]).forEach(i=>{
      const m=String(i.code||'').toUpperCase().match(new RegExp('^'+prefix.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')+'[-_/ ]?(\\\\d+)$'));
  ...
  async function saveNewItem(form){
    ...
    const category=(window.state?.categories||[]).find(c=>String(c.id)===categoryId);
    const code=String(f.get('code')||'').trim().toUpperCase()||nextCode(category);
    ...
    if(initial>0){
      ...
      if(movementError){toastError('El artículo se creó, pero no se pudo registrar el stock inicial: ',movementError);}
    }
    if(typeof closeModal==='function')closeModal();
    if(typeof load==='function')await load();
    if(typeof view==='function')view();
    if(typeof toast==='function')toast('Artículo creado correctamente');
  }

  function wireItemForm(){
    const form=document.querySelector('#item-form');
    if(!form||form.dataset.qfSave)return;
    form.dataset.qfSave='1';
    const code=form.querySelector('[name="code"]');
    const cat=form.querySelector('[name="category_id"]');
    const fill=()=>{if(code&&!code.value&&cat){const c=(window.state?.categories||[]).find(x=>String(x.id)===String(cat.value));code.value=nextCode(c);}};
    fill();
    cat?.addEventListener('change',()=>{if(code)code.value='';fill();});
  ...
  function wireEditForm(){
    ...
    form.addEventListener('submit',async e=>{
      ...
      const f=new FormData(form),itemCode=String(f.get('code')||'').trim().toUpperCase(),name=String(f.get('name')||'').trim(),unit=String(f.get('unit')||'UND').trim().toUpperCase()||'UND',minimum=Number(f.get('minimum'));
      const item=(window.state?.items||[]).find(i=>i.code===form.querySelector('[name="code"]')?.defaultValue)||null;
      if(!item){toastError('No se pudo identificar el artículo para editar. ',null);return;}
      if(!itemCode||!name||!f.get('category_id')||!Number.isFinite(minimum)||minimum<0){toastError('Revisa los campos obligatorios y el stock mínimo. ',null);return;}
      const btn=form.querySelector('button[type="submit"]');if(btn){btn.disabled=true;btn.textContent='Guardando...';}
      const {error}=await sb.from('items').update({code:itemCode,category_id:f.get('category_id'),name,unit,minimum_stock:minimum,description:String(f.get('description')||'').trim()||null}).eq('id',item.id);
  ...
  function cleanDuplicateFicha(){
    ...
    if(!head)return;
    const fichaHeads=[...head.children].filter(th=>th.textContent.trim().toLowerCase()==='ficha');
    if(fichaHeads.length<=1)return;
    const keep=fichaHeads[0];
    const removeIndex=fichaHeads.indexOf(fichaHeads[1]);
  ...
  function tick(){
    try{wireItemForm();wireEditForm();cleanDuplicateFicha();}catch(err){console.error('[QUIMFLUX ui-fix]',err);}
  }
