...
(function(){
  ...
  async function renderDashboard(){
    ...
    const {data,error}=await sb.from('movements').select('id,item_id,movement_type,quantity,movement_date,document_number,supplier,destination,created_by,items(code,name,category_id,category_name)').gte('movement_date',from).order('movement_date',{ascending:false}).limit(5000);
  ...
  function paintDashboard(){
    ...
    const entries=mov.filter(m=>m.movement_type==='ENTRY');
    const exits=mov.filter(m=>m.movement_type==='EXIT');
    const last30=mov.filter(m=>new Date(m.movement_date).getTime()>=since);
    const out30=last30.filter(m=>m.movement_type==='EXIT');
  ...
  function buildReplenishment(items,mov){
    ...
    return items.map(i=>{const consumption=mov.filter(m=>m.item_id===i.id&&m.movement_type==='EXIT'&&new Date(m.movement_date).getTime()>=since).reduce((a,m)=>a+Number(m.quantity||0),0);const daily=consumption/30;const target=Math.max(Number(i.minimum_stock||0)*2,Number(i.minimum_stock||0)+daily*30);const suggested=Math.max(0,target-Number(i.current_stock||0));if(suggested<=0&&i.stock_status==='SUFFICIENT')return null;return {code:i.code,name:i.name,stock:Number(i.current_stock||0),minimum:Number(i.minimum_stock||0),consumption,daily,suggested:Math.ceil(suggested*100)/100,priority:i.stock_status==='CRITICAL'?'CRÍTICA':'ALTA'};}).filter(Boolean).sort((a,b)=>(a.priority==='CRÍTICA'?0:1)-(b.priority==='CRÍTICA'?0:1)||b.suggested-a.suggested);
  }
  function topItems(out){const m={};out.forEach(x=>{const k=x.item_id;if(!m[k])m[k]={code:x.items?.code||'',name:x.items?.name||'',category:x.items?.category_name||'',qty:0};m[k].qty+=Number(x.quantity||0)});return Object.values(m).sort((a,b)=>b.qty-a.qty).slice(0,15);}
  function exportCSV(){
    const items=state.items||[], mov=dashboardData?.movements||[];const rows=[['Código','Bien','Categoría','Stock','Mínimo','Estado','Salidas 30d']];const since=Date.now()-30*86400000;items.forEach(i=>{const q=mov.filter(m=>m.item_id===i.id&&m.movement_type==='EXIT'&&new Date(m.movement_date).getTime()>=since).reduce((a,m)=>a+Number(m.quantity||0),0);rows.push([i.code,i.name,i.category_name||'',i.current_stock,i.minimum_stock,i.stock_status,q])});const csv='\\ufeff'+rows.map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(';')).join('\\r\\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download='QUIMFLUX_gestion_inventario.csv';a.click();URL.revokeObjectURL(a.href);
  }
  wait();
})();