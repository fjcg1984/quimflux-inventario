...
(function(){
  ...
  async function renderDashboard(){
    ...
    // inventory_overview ya entrega category_name; movements solo necesita datos propios del artículo.
    const {data,error}=await sb.from('movements').select('id,item_id,movement_type,quantity,movement_date,document_number,supplier,destination,created_by,items(code,name,category_id)').gte('movement_date',from).order('movement_date',{ascending:false}).limit(5000);
